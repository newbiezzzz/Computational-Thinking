...
function box_blur(v::AbstractArray, l)
	...
	for i in 1:length(a)+1
		...
		count = 1
		# Since this is for middle values only
		if (i <= length(a))
			for b in 1:l
				j += 1 # Going to >>
				k -= 1 # Going to <<
				# If j and k is not out of bound || Range is a[1] to a[length(a)]
				if k >= 1 && j <= length(a)
					a[i] += v[j] + v[k]
					count += 2
				# If k is out of bound || Range is 
				elseif k < 1  && j <= length(a)
					a[i] += v[j]
					count += 1
				# If j is out of bound
				elseif j > length(a) && k >= 1
					a[i] += v[k]
					count += 1
				end
			end
...
# ╔═╡ f1deab6b-1e1f-45f7-86de-a7331d29f8e2
box_blur([1,2,3], 1)
...
extend([5,6,19,7,1], 19) # return 19

# ╔═╡ 80479d98-ee09-11ea-169e-d166eef65874
extend([5,6,7], -8) # return 5

# ╔═╡ 805691ce-ee09-11ea-053d-6d2e299ee123
extend([6,7],-10)  # return 7
...
function convolve(v::AbstractVector, k)
	a = fill(0.0, length(v))
	l = (length(k)-1)/2
    value = convert(Int, l + 1)
	for i in 1:length(v)
		for m in 1:length(k)
			a[i] += extend(v, i-m+value) * k[m]
		end
	end
	return a
end

# ╔═╡ 4d3f7f0a-e6b6-41fe-8349-5c312401a0a0
a = [1 2 3 ; 1 2 3]

# ╔═╡ fa92542d-25d0-4bd8-b443-f2c3cfc3b949
size(a), length(a)

# ╔═╡ 6fb5a1f0-5a1d-4723-88eb-f3f5c81139fb
extend(a, -1, -100)
...
function convolve(M::AbstractMatrix, K::AbstractMatrix)
	M_num_rows, M_num_columns = size(M)
	K_num_rows, K_num_columns = size(K)
	# For RGB image
	if typeof(M) == Matrix{RGB{N0f8}}
	...
	a = fill(element, M_num_rows, M_num_columns)
	n = convert(Int, (K_num_rows - 1) / 2)
	...
	if typeof(M) == Matrix{RGB{N0f8}}
		...
		g = green.(M)
		b = blue.(M)
		for i in 1:M_num_rows
			for j in 1:M_num_columns
				for k in -n:n
					for l in -n:n
						r1[i,j] += extend(r, i+k, j+l) * K[k+1+n,l+1+n]
						g1[i,j] += extend(g, i+k, j+l) * K[k+1+n,l+1+n]
						b1[i,j] += extend(b, i+k, j+l) * K[k+1+n,l+1+n]
					end
				end
			end
		end
		return RGB.(r1,g1,b1)
	# For grayscale or just normal matrix
	else
		for i in 1:M_num_rows
			for j in 1:M_num_columns
				for k in -n:n
					for l in -n:n
						a[i,j] += extend(M, i+k, j+l) * K[k+1+n,l+1+n]
					end
				end
			end
		end
...
# ╔═╡ 6ce97b76-a8e5-46bf-8298-3f8accff04a1
typeof(philip_head)

# ╔═╡ 2f82aeaf-0890-42ba-8066-fd6b9049935f
typeof(Gray.(philip_head))
...
# ╔═╡ 9951ab60-b489-45a0-a556-3d168e957527
convolve(K_test, K_test)
...
# ╔═╡ f71f845b-7598-47cc-b57d-802e87da348c
philip_head
...
# ╔═╡ cb031646-4c6e-43e6-9f3d-9f8b002a10e2
gauss(x::Real, y::Real ; σ=1) = 1 / sqrt(2π*σ^2) * exp(-(x^2+y^2) / (2 * σ^2))
...
# ╔═╡ e78b9d21-8a4a-4b41-b01b-ac2ed92d79f4
(gaussian_kernel_1D(4; σ=10))
...
function with_gaussian_blur(image; σ=1, l=5)
	a = fill(0.0, l, l)
	n = convert(Int, (l-1)/2)
	x = 0
	for i in -n:n
...
# ╔═╡ 240121a8-4d00-4937-b3d9-f272c5a0f90e
philip_head
...
function with_sobel_edge_detect(image)
	...
	img = Gray.(image)
	# More weight at the middle, less weight at the top/bottom
	Gx = [1 0 -1
		  2 0 -2
		  1 0 -1]
	...
	#a = map.(x -> x^4, convolve(img, Gx)) # power the matrix by x
	#b = map.(x -> x^4, convolve(img, Gy)) # power the matrix by x
	a = convolve(img, Gx).^4
	b = convolve(img, Gy).^4
	G = Gray.(sqrt.(a + b))
	
	# To conclude what i have learnt is:
	# The magnitude of change (Gx^2 / Gy^2)
	# is the change of intensity/brightness.
end

# ╔═╡ b432f01e-e756-4914-ac6a-753cb1b9315c
