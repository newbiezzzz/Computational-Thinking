...
function remove_in_each_row(img::Matrix, column_numbers::Vector)
	m, n = size(img) # m is horizontal(row), n is vertical(col)
	...
	for (i, j) in enumerate(column_numbers) # i = 1,2,3,4..., j is the given array.
		img′[i, 1:j-1] .= @view img[i, 1:(j-1)] # 1:j - 1 == 1:(j - 1)
		img′[i, j:end] .= @view img[i, (j+1):end]
		# Note img' = img (See carefully)
	end
	img′
end
...
random_seam(m, n, i) = reduce((a, b) -> [a..., clamp(last(a) + rand(-1:1), 1, n)], 1:m-1; init=[i]) #clamp(x,lo,hi)
...
function greedy_seam(energies, starting_pixel::Int)
	m, n = size(energies) #energies = image/matrix, m is height, n is width.
	
	# you can delete the body of this function - it's just a placeholder.
	#random_seam(size(energies)..., starting_pixel)
	
	a = fill(starting_pixel, m)
	p = starting_pixel
	for i in 2:m
		a[i] = p
		if p > 1 && p < n && energies[i,p-1] < energies[i,p] && energies[i,p-1] < energies[i,p+1] 
            #("left")
			a[i] = p-1
		elseif p > 1 && p < n && energies[i,p+1] < energies[i,p] && energies[i,p+1] < energies[i,p-1] 
            #("right")
			a[i] = p+1
        elseif p == 1 && n != 1 && energies[i,p+1] < energies[i,p]
            #("right")
            a[i] = p+1
        elseif p == n && n != 1 && energies[i,p-1] < energies[i,p]
            #("left")
            a[i] = p-1
		end
		p = a[i]
