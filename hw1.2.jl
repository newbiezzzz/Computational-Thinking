...
function quantize(color::AbstractRGB)
	r = quantize(red(color))
	g = quantize(green(color))
	b = quantize(blue(color))
	# your code here!
	return RGB(r,g,b)
end
...
function quantize(image::AbstractMatrix)
	# your code here!
	r = quantize.(red.(image))
	g = quantize.(green.(image))
	b = quantize.(blue.(image))
	return RGB.(r,g,b)
end
...
function noisify(x::Number, s)
	# your code here!
	randomed_value = clamp(randn(Float64), -s , s)
	missing = clamp(x + randomed_value, 0 , 1)
	return missing
end
...
function noisify(color::AbstractRGB, s)
	# your code here!
	r = noisify(red(color), 0.3)
	g = noisify(green(color), 0.3)
	b = noisify(blue(color), 0.3)
	return RGB(r,g,b)
end
...
function noisify(image::AbstractMatrix, s)
	# your code here!
	r = noisify.(red.(image), 1)
	g = noisify.(green.(image), 1)
	b = noisify.(blue.(image), 1)
	return RGB.(r,g,b)
end
