...
# ╔═╡ 8a995926-63f0-4309-accc-502e0747be3a
transition_counts(first_sample)
...
# ╔═╡ 6192ba92-fad1-422a-b497-27307d8d2391
function doublel(matrix)
	a = diag(matrix)
	count = 0
	for i in 1:length(a)
		if a[i] != 0.0
...
# ╔═╡ 80a3271f-187b-450d-94a9-98622633f090
doublel(sample_freq_matrix)

# ╔═╡ e03bd58c-49d2-4442-8318-edb7b8ee6d08
diag(sample_freq_matrix)
...
function matrix_distance(A, B)
	...
	return sum # do something with A .- B
end
...
emma_count = get(word_counts(emma_words), "Emma", 0)
...
function completion_cache(grams)
	...
	for word in grams
		i += 1
		if haskey(cache, (grams[i][1], grams[i][2])) == true
			cache[grams[i][1], grams[i][2]] = cache[grams[i][1], grams[i][2]], grams[i][3]
		else
			cache[grams[i][1], grams[i][2]] = grams[i][3]
		end
	end
